...
async def simulate_edge(request: OddsRequest):
    ...
    # Tier 2: Poisson Check (Mocked for now with a constant lambda based on odds)
    # In production, lambdas would come from xG models
    mock_lambda = 1.5 
    poisson_edge = calculate_poisson_edge(mock_lambda, mock_lambda, 2)
...
async def execute_v11_analysis(request: OddsRequest):
    ...
    prop_prob = run_monte_carlo_player_prop(star_player, star_avg, target_line)
